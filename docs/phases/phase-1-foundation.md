...
- [x] Scheduling wired into the heartbeat loop (`aprendi/app/scheduler.py`):
  first attempt Saturday 2am, retry Sunday 2am if occupied, using a
  correct Europe/Lisbon timezone (DST-aware, via zoneinfo). Dedup logic
...
- [x] Automatic desktop shutdown after a successful readiness check
  (`desktop.shutdown()`, via a dedicated SSH keypair), verified
  end-to-end: desktop woke, confirmed ready, powered itself off with no
  manual step. **The actual brief-generation pipeline still does not
  exist** — the desktop currently just shuts back down right after being
  confirmed ready.
